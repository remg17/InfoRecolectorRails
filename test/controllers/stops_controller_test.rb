...
class StopsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create stop" do
    assert_difference('Stop.count') do
      post stops_url, params: { stop: { address: @stop.address, latitude: @stop.latitude, length: @stop.length } }, as: :json
    end
  ...
  test "should update stop" do
    patch stop_url(@stop), params: { stop: { address: @stop.address, latitude: @stop.latitude, length: @stop.length } }, as: :json
    assert_response 200
  end
